...
const Game = () => {
  ...
  const deleteSavedCards = async () => {
    try {
      const response = await fetch(
        `http://localhost:8080/api/deleteSavedCards?username=${username}`,
        {
  ...
  const fetchSavedCards = async () => {
    try {
      const response = await fetch(
        `http://localhost:8080/api/fetchSavedCards?username=${username}`,
        {
      ...
      console.log("Fetched saved cards:", cards); // Do something with the fetched cards
  ...
  const saveCardDraw = async (username, cardType) => {
    try {
      await fetch(
        `http://localhost:8080/api/saveCardDraw?username=${username}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
